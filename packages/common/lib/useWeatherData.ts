    ...
    for (let i = 0; i < geoObjectsArray?.length; i++) {
      ...
      try {
        const response = await axios.get(
          `https://api.open-meteo.com/v1/forecast?latitude=${geoObjectsArray[i]["lat"]}&longitude=${geoObjectsArray[i]["lng"]}&hourly=temperature_2m,precipitation${timezoneQuery}&timeformat=unixtime`
        );
